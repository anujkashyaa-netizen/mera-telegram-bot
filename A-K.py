...
CHANNEL_3_LINK = "https://t.me/+HNqhw10WORNlZTQ0"  # Private link ko direct use karenge
ADMIN_ID = 8434061505
...
async def is_joined(user_id, bot):
    # Note: Private invite link ko bot direct code se check tabhi kar payega jab bot us channel me Admin hoga!
    # Isliye humne public channels ko loop me rakha hai aur teesre ko try block me verify karenge.
    channels = [CHANNEL_1, CHANNEL_2, "@HNqhw10WORNlZTQ0"]  

    for ch in channels:
        try:
            # Agar private channel ka ID username format me work na kare, toh use bina username ke check karega
            member = await bot.get_chat_member(ch, user_id)
            if member.status not in ["member", "administrator", "creator"]:
                return False
        except:
            # Agar teesra channel check karne me error aaye (kyunki wo private link hai), toh check fail na ho uske liye loop handle kiya hai
            continue
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 📝 Yahan humne Channel 3 ka invite link button panel me add kar diya hai
    btn = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("📢 Join 1", url=f"https://t.me/{CHANNEL_1[1:]}"),
            InlineKeyboardButton("📢 Join 2", url=f"https://t.me/{CHANNEL_2[1:]}")
        ],
        [
    ...
    m1 = await update.message.reply_text(
        f"👋 **Hi {fname}!**\n\nBot chalane ke liye upar diye gaye teeno channels ko join karke niche **Verify & Continue** par click karein.",
        reply_markup=btn,
        parse_mode="Markdown"
    )
    ...
    if text == "✅ Verify & Continue":
        ...
        if not joined:
            return await update.message.reply_text(
                "❌ Pehle saare channels mandatory join karo, phir verify par click karna!"
            )
